(function (global, factory) {
	...
	var objectInspect = /*#__PURE__*/Object.freeze({
		__proto__: null
	});

	var require$$2 = /*@__PURE__*/getAugmentedNamespace(objectInspect);
	  ...
	  if (obj instanceof Array) {
	    for (let i = 0; i < obj.length; i++) {
	      ...
	      if (rbracket.test(key)) {
	        ...
	        buildParams(
	          key + "[" + (typeof value === "object" && value != null ? i : "") + "]",
	          value,
	          add
	        );
	      }
	...
	const cardApi = {
	  ...
	  expiry(value) {
	    ...
	    if (year && year.length === 2 && /^\d+$/.test(year)) {
	      const prefix = (/* @__PURE__ */ new Date()).getFullYear().toString().substring(0, 2);
	      year = prefix + year;
	    }
	    return {
	      month: ~~month,
	  ...
	  validateExpiry(month, year) {
	    let r, i;
	    return month = String(month).trim(), year = String(year).trim(), /^\d+$/.test(month) ? /^\d+$/.test(year) ? parseInt(month, 10) <= 12 ? (i = new Date(year, month), r = /* @__PURE__ */ new Date(), i.setMonth(i.getMonth() - 1), i.setMonth(i.getMonth() + 1, 1), i > r) : false : false : false;
	  },
	  validateCVC(val) {
	...
	const cacheApi = {
	  ...
	  set(details) {
	    ...
	    if (path || value instanceof Array) {
	      let upData = __spreadValues$l({}, data || {});
	...
	function getProductOptionIndex(product, filter = void 0) {
	  ...
	  return reduce(
	    ...
	    (acc, op) => {
	      const values = reduce(
	        op.values,
	        (acc2, val) => __spreadProps$b(__spreadValues$k({}, acc2), {
	          [val.id]: __spreadProps$b(__spreadValues$k({}, val), { id: val.id }),
	          [val.name]: __spreadProps$b(__spreadValues$k({}, val), { id: val.id })
	        }),
	        {}
	      );
	      return __spreadProps$b(__spreadValues$k({}, acc), {
	        [op.id]: __spreadProps$b(__spreadValues$k({}, op), { values }),
	        [op.name]: __spreadProps$b(__spreadValues$k({}, op), { values })
	      });
	    },
	    {}
	  );
	...
	function calculateVariation(input, options, purchaseOption) {
	  ...
	  const variation = __spreadProps$b(__spreadValues$k({}, product), {
	...
	function findPurchaseOption(product, purchaseOption) {
	  ...
	  if (option) {
	    ...
	    return __spreadProps$b(__spreadValues$k({}, option), {
	...
	async function getFilterableAttributeFilters(request, products, options) {
	  ...
	  return getFilters(products, __spreadProps$b(__spreadValues$k({}, options), { filterableAttributes }));
	}
	      ...
	      if (product.attributes) {
	        for (let id in product.attributes) {
	          ...
	          if (attr) {
	            ...
	            attributes.push(__spreadProps$b(__spreadValues$k({}, product.attributes[id]), {
	              value: void 0,
	  ...
	  return {
	    ...
	    getItemData(item, data = {}) {
	      ...
	      if (typeof item === "string") {
	        result = __spreadProps$a(__spreadValues$j({}, data || {}), {
	          product_id: item
	        });
	      }
	    ...
	    update(input) {
	      ...
	      if (data.items && data.items.map) {
	        data = __spreadProps$a(__spreadValues$j({}, data), {
	  ...
	  return {
	    get: (type, id, query) => {
	      return cacheApi.getFetch(
	        `content_${type}`,
	        id,
	        () => request("get", `/content/${type}`, id, __spreadValues$i({
	  ...
	  return {
	    ...
	    findState(uri, stateName, { where = void 0, def = void 0 } = {}) {
	      ...
	      if (state && typeof state.then === "function") {
	        return state.then((state2) => find$1(state2, where) || def);
	      }
	    ...
	    decodeLocale(values) {
	      ...
	      if (configs) {
	        configs = configs.reduce(
	          (acc, config) => __spreadProps$9(__spreadValues$h({}, acc), {
	            [config.code]: config
	          }),
	          {}
	        );
	...
	function decodeLocaleValue(locale, values, key, configs, opt) {
	  ...
	  if (returnLocaleConfig) {
	    ...
	    while (fallbackKey) {
	      ...
	      fallbackValues = __spreadValues$h(__spreadValues$h({}, values[key][fallbackKey] || {}), fallbackValues);
	  ...
	  let localeValues = __spreadValues$h(__spreadValues$h({}, fallbackValues), values[key][returnLocaleKey] || {});
	...
	class Payment {
	  ...
	  async _ensureCartSettings(cart) {
	    ...
	    return __spreadProps$8(__spreadValues$g({}, cart), { settings: __spreadValues$g({}, settings.store) });
	  }
	  /**
	  ...
	  async _populateScriptWithCartParams(script) {
	    ...
	    const cart = await this.getCart();
	    script.params = __spreadValues$g(__spreadValues$g({}, script.params), pick$1(cart, cartParams));
	...
	function getBillingDetails(cart) {
	  const details = __spreadValues$f({}, mapValues(billingFieldsMap, cart.billing));
	...
	function setBancontactOwner(source, data) {
	  ...
	  const billingData = __spreadValues$f(__spreadValues$f(__spreadValues$f(__spreadValues$f({}, account.shipping), account.billing), shipping), billing);
	  const billingAddress = fillValues(addressFieldsMap$1, billingData);
	  source.owner = __spreadValues$f(__spreadValues$f({
	    email: account.email,
	...
	async function createIDealPaymentMethod(stripe, element, cart) {
	  const billingDetails = getBillingDetails(cart);
	  return await stripe.createPaymentMethod(__spreadValues$f({
	    type: "ideal",
	    ideal: element
	...
	function getKlarnaConfirmationDetails(cart) {
	  const billingDetails = getBillingDetails(cart);
	  const returnUrl = `${window.location.origin + window.location.pathname}?gateway=stripe`;
	  return {
	    payment_method: {
	      billing_details: billingDetails
	    },
	...
	class StripeIDealPayment extends Payment {
	  constructor(request, options, params, methods) {
	    ...
	    const method = __spreadProps$7(__spreadValues$d({}, methods.ideal), {
	...
	class StripeBancontactPayment extends Payment {
	  constructor(request, options, params, methods) {
	    ...
	    const method = __spreadProps$6(__spreadValues$c({}, methods.bancontact), {
	      publishable_key: methods.card.publishable_key
	    });
	    super(request, options, params, method);
	  }
	...
	class StripeKlarnaPayment extends Payment {
	  constructor(request, options, params, methods) {
	    ...
	    const method = __spreadProps$5(__spreadValues$b({}, methods.klarna), {
	      publishable_key: methods.card.publishable_key
	    });
	    super(request, options, params, method);
	  }
	...
	class StripeGooglePayment extends Payment {
	  constructor(request, options, params, methods) {
	    ...
	    const method = __spreadProps$4(__spreadValues$a({}, methods.google), {
	      publishable_key: methods.card.publishable_key
	    });
	    super(request, options, params, method);
	  }
	  ...
	  async createElements(cart) {
	    ...
	    if (!(canMakePayment == null ? void 0 : canMakePayment.googlePay)) {
	      throw new Error(
	  ...
	  _createPaymentRequest(cart) {
	    ...
	    const paymentRequest = this.stripe.paymentRequest(__spreadValues$a({
	  ...
	  async _onShippingAddressChange(event) {
	    ...
	    const cart = await this.updateCart({
	      shipping: __spreadProps$4(__spreadValues$a({}, shipping), { service: null }),
	      shipment_rating: null
	    });
	    if (cart) {
	      updateWith(__spreadValues$a({
	        status: "success"
	      }, getPaymentRequestData(cart, this.params)));
	    } else {
	  ...
	  async _onPaymentMethod(event) {
	    ...
	    await this.updateCart(__spreadProps$4(__spreadValues$a({
	      account: {
	        email: payerEmail
	      }
	    }, requireShipping && {
	      shipping: __spreadProps$4(__spreadValues$a({}, this._mapShippingAddress(shippingAddress)), {
	        service: shippingOption.id
	      })
	    }), {
	      billing: __spreadProps$4(__spreadValues$a({}, this._mapBillingAddress(billing_details)), {
	        method: "card",
	        card: {
	          gateway: "stripe",
	          token: paymentMethod,
	  ...
	  _getButtonStyles() {
	    let { style: { color = "dark", type = "default", height = "45px" } = {} } = this.params;
	    switch (color) {
	      case "white":
	        color = "light";
	        break;
	...
	class StripeApplePayment extends Payment {
	  constructor(request, options, params, methods) {
	    ...
	    const method = __spreadProps$3(__spreadValues$9({}, methods.apple), {
	      publishable_key: methods.card.publishable_key
	    });
	    super(request, options, params, method);
	  }
	  ...
	  _createPaymentRequest(cart) {
	    const { require: { name, email, shipping, phone } = {} } = this.params;
	    const paymentRequest = this.stripe.paymentRequest(__spreadValues$9({
	      requestPayerName: Boolean(name),
	      requestPayerEmail: Boolean(email),
	      requestPayerPhone: Boolean(phone),
	      requestShipping: Boolean(shipping),
	  ...
	  async _onShippingAddressChange(event) {
	    ...
	    const shipping = this._mapShippingAddress(shippingAddress);
	    const cart = await this.updateCart({
	      shipping: __spreadProps$3(__spreadValues$9({}, shipping), { service: null }),
	      shipment_rating: null
	    });
	    if (cart) {
	      updateWith(__spreadValues$9({
	        status: "success"
	      }, getPaymentRequestData(cart, this.params)));
	    } else {
	      updateWith({ status: "invalid_shipping_address" });
	  ...
	  async _onPaymentMethod(event) {
	    ...
	    const { require: { shipping: requireShipping } = {} } = this.params;
	    await this.updateCart(__spreadProps$3(__spreadValues$9({
	      account: {
	        email: payerEmail
	      }
	    }, requireShipping && {
	      shipping: __spreadProps$3(__spreadValues$9({}, this._mapShippingAddress(shippingAddress)), {
	        service: shippingOption.id
	      })
	    }), {
	      billing: __spreadProps$3(__spreadValues$9({}, this._mapBillingAddress(billing_details)), {
	        method: "card",
	        card: {
	          gateway: "stripe",
	          token: paymentMethod,
	...
	const mapAddressFields = (cart, addressField) => __spreadProps$2(__spreadValues$5({}, mapFields(addressFieldsMap, cart[addressField])), {
	    ...
	    try {
	      ...
	      if (selected_shipping_option) {
	        selectedShippingService = shippingServices.find(
	          (shippingService) => shippingService.id === selected_shipping_option.id
	        );
	      }
	...
	class AmazonDirectPayment extends Payment {
	  ...
	  _createSession(cart) {
	    ...
	    return this.authorizeGateway({
	      gateway: "amazon",
	      params: __spreadProps$1(__spreadValues$3({
	...
	function adjustParams(_params) {
	  const params = __spreadValues$2({}, _params);
	  adjustConfig(params);
	  return params;
	}
	function adjustMethodParams(_methodParams) {
	  const methodParams = __spreadValues$2({}, _methodParams);
	...
	function methods(request, _opt) {
	  return {
	     ...
	     * @returns {any}
	     */
	    request(method, appId, functionName, data, options = void 0) {
	      return request(method, functionName, void 0, data, __spreadProps(__spreadValues$1({}, options), {
	     ...
	     * @returns {any}
	     */
	    get(appId, functionName, data, options = void 0) {
	     ...
	     * @returns {any}
	     */
	    put(appId, functionName, data, options = void 0) {
	     ...
	     * @returns {any}
	     */
	    post(appId, functionName, data, options = void 0) {
	     ...
	     * @returns {any}
	     */
	    delete(appId, functionName, data, options = void 0) {
	...
	async function request(method, url, id = void 0, data = void 0, opt = void 0) {
	  ...
	  const session2 = allOptions.session || allOptions.getCookie("swell-session");
	  const locale2 = allOptions.locale || allOptions.getCookie("swell-locale");
	  const currency2 = allOptions.currency || allOptions.getCookie("swell-currency");
	  ...
	  if (typeof responseSession === "string" && session2 !== responseSession) {

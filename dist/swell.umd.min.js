(function (global, factory) {
	...
	const cacheApi = {
	  ...
	  set(details) {
	    ...
	    if (path || value instanceof Array) {
	      let upData = __spreadValues$g({}, data || {});
	  ...
	  return reduce$1(
	    ...
	    (acc, op) => {
	      const values = reduce$1(
	        op.values,
	        (acc2, val) => __spreadProps$b(__spreadValues$f({}, acc2), {
	          [val.id]: __spreadProps$b(__spreadValues$f({}, val), { id: val.id }),
	          [val.name]: __spreadProps$b(__spreadValues$f({}, val), { id: val.id })
	        }),
	        {}
	      );
	      return __spreadProps$b(__spreadValues$f({}, acc), {
	        [op.id]: __spreadProps$b(__spreadValues$f({}, op), { values }),
	        [op.name]: __spreadProps$b(__spreadValues$f({}, op), { values })
	      });
	    },
	    {}
	  );
	...
	function calculateVariation(input, options, purchaseOption) {
	  ...
	  const variation = __spreadProps$b(__spreadValues$f({}, product), {
	...
	function findPurchaseOption(product, purchaseOption) {
	  ...
	  if (option) {
	    ...
	    return __spreadProps$b(__spreadValues$f({}, option), {
	...
	async function getFilterableAttributeFilters(request, products, options) {
	  ...
	  return getFilters(products, __spreadProps$b(__spreadValues$f({}, options), { filterableAttributes }));
	}
	      ...
	      if (product.attributes) {
	        for (let id in product.attributes) {
	          ...
	          if (attr) {
	            ...
	            attributes.push(__spreadProps$b(__spreadValues$f({}, product.attributes[id]), {
	              value: void 0,
	  ...
	  return {
	    ...
	    getItemData(item, data = {}) {
	      ...
	      if (typeof item === "string") {
	        result = __spreadProps$a(__spreadValues$e({}, data || {}), {
	          product_id: item
	        });
	      }
	    ...
	    update(input) {
	      ...
	      if (data.items && data.items.map) {
	        data = __spreadProps$a(__spreadValues$e({}, data), {
	  ...
	  return {
	    get: (type, id, query) => {
	      return cacheApi.getFetch(
	        `content_${type}`,
	        id,
	        () => request("get", `/content/${type}`, id, __spreadValues$d({
	    ...
	    decodeLocale(values) {
	      ...
	      if (configs) {
	        configs = configs.reduce(
	          (acc, config) => __spreadProps$9(__spreadValues$c({}, acc), {
	            [config.code]: config
	          }),
	          {}
	        );
	...
	function decodeLocaleValue(locale, values, key, configs, opt) {
	  ...
	  if (returnLocaleConfig) {
	    ...
	    while (fallbackKey) {
	      ...
	      fallbackValues = __spreadValues$c(__spreadValues$c({}, values[key][fallbackKey] || {}), fallbackValues);
	  ...
	  let localeValues = __spreadValues$c(__spreadValues$c({}, fallbackValues), values[key][returnLocaleKey] || {});
	...
	class StripeIDealPayment extends Payment {
	  constructor(request, options, params, methods) {
	    ...
	    const method = __spreadProps$8(__spreadValues$9({}, methods.ideal), {
	...
	class StripeBancontactPayment extends Payment {
	  constructor(request, options, params, methods) {
	    ...
	    const method = __spreadProps$7(__spreadValues$8({}, methods.bancontact), {
	      publishable_key: methods.card.publishable_key
	    });
	    super(request, options, params, method);
	  }
	...
	class StripeKlarnaPayment extends Payment {
	  constructor(request, options, params, methods) {
	    ...
	    const method = __spreadProps$6(__spreadValues$7({}, methods.klarna), {
	      publishable_key: methods.card.publishable_key
	    });
	    super(request, options, params, method);
	  }
	  ...
	  async tokenize() {
	    ...
	    const { source, error: sourceError } = await createKlarnaSource(
	      this.stripe,
	      __spreadProps$6(__spreadValues$7({}, cart), {
	...
	class StripeGooglePayment extends Payment {
	  constructor(request, options, params, methods) {
	    ...
	    const method = __spreadProps$5(__spreadValues$6({}, methods.google), {
	      publishable_key: methods.card.publishable_key
	    });
	    super(request, options, params, method);
	  }
	  ...
	  async _submitPayment(cart, paymentData) {
	    ...
	    await this.updateCart(__spreadValues$6(__spreadProps$5(__spreadValues$6({}, shouldUpdateAccount && {
	      account: {
	        email,
	...
	class StripeApplePayment extends Payment {
	  constructor(request, options, params, methods) {
	    ...
	    const method = __spreadProps$4(__spreadValues$5({}, methods.apple), {
	      publishable_key: methods.card.publishable_key
	    });
	    super(request, options, params, method);
	  }
	  ...
	  async _onShippingAddressChange(event) {
	    ...
	    const cart = await this.updateCart({
	      shipping: __spreadProps$4(__spreadValues$5({}, shipping), { service: null }),
	  ...
	  async _onPaymentMethod(cart, event) {
	    ...
	    const { require: { shipping: requireShipping } = {} } = this.params;
	    const shouldUpdateAccount = !Boolean(cart.account && cart.account.email);
	    await this.updateCart(__spreadProps$4(__spreadValues$5(__spreadValues$5({}, shouldUpdateAccount && {
	      account: {
	        name: payerName,
	        email: payerEmail
	      }
	    }), requireShipping && {
	      shipping: __spreadProps$4(__spreadValues$5({}, this._mapShippingAddress(shippingAddress)), {
	        service: shippingOption.id
	      })
	    }), {
	      billing: __spreadProps$4(__spreadValues$5({}, this._mapBillingAddress(billing_details)), {
	        method: "card",
	        card: {
	          gateway: "stripe",
	          token: paymentMethod,
	...
	class BraintreeGooglePayment extends Payment {
	  ...
	  async _submitPayment(cart, googlePayment, paymentData) {
	    ...
	    await this.updateCart(__spreadValues$4(__spreadProps$3(__spreadValues$4({}, shouldUpdateAccount && {
	      account: {
	        email,
	        name: shippingAddress ? shippingAddress.name : billingAddress.name
	      }
	...
	class BraintreeApplePayment extends Payment {
	  ...
	  _createPaymentSession(cart, applePayment, paymentRequest) {
	    ...
	    session.onpaymentauthorized = async (event) => {
	      ...
	      await this.updateCart(__spreadValues$3(__spreadProps$2(__spreadValues$3({}, shouldUpdateAccount && {
	        account: {
	...
	class PaypalDirectPayment extends Payment {
	  ...
	  async _onApprove(cart, data, actions) {
	    ...
	    await this.updateCart(__spreadProps(__spreadValues$1({}, usePayPalEmail && {
	      account: {
	...
	async function request$1(method, url, id = void 0, data = void 0, opt = void 0) {

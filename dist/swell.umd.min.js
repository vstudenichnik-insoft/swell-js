(function (global, factory) {
	...
	function getProductOptionIndex(product, filter = void 0) {
	  ...
	  return reduce(
	    ...
	    (acc, op) => {
	      const values = reduce(
	        op.values,
	        (acc2, val) => __spreadProps$9(__spreadValues$g({}, acc2), {
	          [val.id]: __spreadProps$9(__spreadValues$g({}, val), { id: val.id }),
	          [val.name]: __spreadProps$9(__spreadValues$g({}, val), { id: val.id })
	        }),
	        {}
	      );
	      return __spreadProps$9(__spreadValues$g({}, acc), {
	        [op.id]: __spreadProps$9(__spreadValues$g({}, op), { values }),
	        [op.name]: __spreadProps$9(__spreadValues$g({}, op), { values })
	      });
	    },
	    {}
	  );
	...
	function calculateVariation(input, options, purchaseOption) {
	  ...
	  const variation = __spreadProps$9(__spreadValues$g({}, product), {
	...
	function findPurchaseOption(product, purchaseOption) {
	  ...
	  if (option) {
	    ...
	    return __spreadProps$9(__spreadValues$g({}, option), {
	...
	async function getFilterableAttributeFilters(request, products, options) {
	  ...
	  return getFilters(products, __spreadProps$9(__spreadValues$g({}, options), { filterableAttributes }));
	}
	      ...
	      if (product.attributes) {
	        for (let id in product.attributes) {
	          ...
	          if (attr) {
	            ...
	            attributes.push(__spreadProps$9(__spreadValues$g({}, product.attributes[id]), {
	              value: void 0,
	  ...
	  return {
	    ...
	    getItemData(item, data = {}) {
	      ...
	      if (typeof item === "string") {
	        result = __spreadProps$8(__spreadValues$f({}, data || {}), {
	          product_id: item
	        });
	      }
	    ...
	    update(input) {
	      ...
	      if (data.items && data.items.map) {
	        data = __spreadProps$8(__spreadValues$f({}, data), {
	    ...
	    decodeLocale(values) {
	      ...
	      if (configs) {
	        configs = configs.reduce(
	          (acc, config) => __spreadProps$7(__spreadValues$d({}, acc), {
	            [config.code]: config
	          }),
	          {}
	        );

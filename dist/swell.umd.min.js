(function (global, factory) {
	  ...
	  return reduce$1(
	    ...
	    (acc, op) => {
	      const values = reduce$1(
	        op.values,
	        (acc2, val) => __spreadProps$9(__spreadValues$f({}, acc2), {
	          [val.id]: __spreadProps$9(__spreadValues$f({}, val), { id: val.id }),
	          [val.name]: __spreadProps$9(__spreadValues$f({}, val), { id: val.id })
	        }),
	        {}
	      );
	      return __spreadProps$9(__spreadValues$f({}, acc), {
	        [op.id]: __spreadProps$9(__spreadValues$f({}, op), { values }),
	        [op.name]: __spreadProps$9(__spreadValues$f({}, op), { values })
	      });
	    },
	    {}
	  );
	...
	function calculateVariation(input, options, purchaseOption) {
	  ...
	  const variation = __spreadProps$9(__spreadValues$f({}, product), {
	...
	function findPurchaseOption(product, purchaseOption) {
	  ...
	  if (option) {
	    ...
	    return __spreadProps$9(__spreadValues$f({}, option), {
	...
	async function getFilterableAttributeFilters(request, products, options) {
	  ...
	  return getFilters(products, __spreadProps$9(__spreadValues$f({}, options), { filterableAttributes }));
	}
	      ...
	      if (product.attributes) {
	        for (let id in product.attributes) {
	          ...
	          if (attr) {
	            ...
	            attributes.push(__spreadProps$9(__spreadValues$f({}, product.attributes[id]), {
	              value: void 0,
	  ...
	  return {
	    ...
	    getItemData(item, data = {}) {
	      ...
	      if (typeof item === "string") {
	        result = __spreadProps$8(__spreadValues$e({}, data || {}), {
	          product_id: item
	        });
	      }
	    ...
	    update(input) {
	      ...
	      if (data.items && data.items.map) {
	        data = __spreadProps$8(__spreadValues$e({}, data), {
	    ...
	    decodeLocale(values) {
	      ...
	      if (configs) {
	        configs = configs.reduce(
	          (acc, config) => __spreadProps$7(__spreadValues$c({}, acc), {
	            [config.code]: config
	          }),
	          {}
	        );
	...
	class StripeKlarnaPayment extends Payment {
	  ...
	  async tokenize() {
	    ...
	    const { source, error: sourceError } = await createKlarnaSource(
	      this.stripe,
	      __spreadProps$4(__spreadValues$7({}, cart), {
	...
	class StripeGooglePayment extends Payment {
	  ...
	  async _submitPayment(paymentData) {
	    ...
	    const { card } = token;
	    this.onSuccess(__spreadValues$6({
	      account: {
	        email,
	...
	class StripeApplePayment extends Payment {
	  ...
	  async _onPaymentMethod(event) {
	    ...
	    const { require: { shipping: requireShipping } = {} } = this.params;
	    this.onSuccess(__spreadProps$2(__spreadValues$5({
	      account: {
	        name: payerName,
	        email: payerEmail
	      }
	...
	class BraintreeGooglePayment extends Payment {
	  ...
	  async _submitPayment(googlePayment, paymentData) {
	    ...
	    this.onSuccess(__spreadValues$4({
	      account: {
	        email,
	        name: shippingAddress ? shippingAddress.name : billingAddress.name
	      },
	...
	class BraintreeApplePayment extends Payment {
	  ...
	  _createPaymentSession(applePayment, paymentRequest) {
	    ...
	    session.onpaymentauthorized = async (event) => {
	      ...
	      this.onSuccess(__spreadValues$3({
	        account: {
